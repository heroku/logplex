...
logplex_api_test_() ->
    ...
    {setup,
        fun inets:start/0,
        [{"Healthcheck", 
            ?_assertMatch({ok,{{_,200,_},_,"OK"}}, http:request(get, {"http://localhost:8002/healthcheck", headers()}, [], []))},
         {"Cloudkick",
            ?_assertMatch({ok,{{_,200,_},_,"{\"state\":\"ok\",\"metrics\":[]}"}}, http:request(get, {"http://localhost:8002/cloudkick", headers()}, [], []))},
        ...
        {"Poorly formatted channel name for create channel",
         ...
         {"Create app token",
            fun() ->
                Ret = http:request(post, 
                    {"http://localhost:8002/channels/" ++ get(channel_id) ++ "/token", headers(), content_type(), iolist_to_binary(mochijson2:encode({struct, [
         ...
         {"Upgrade to advanced addon",
            ?_assertMatch({ok,{{_,200,_},_,"OK"}},
                http:request(post, 
                    {"http://localhost:8002/channels/" ++ get(channel_id) ++ "/addon", headers(), content_type(), iolist_to_binary(mochijson2:encode({struct, [
                        {"addon", <<"advanced">>}
                    ]}))}, [], []))},
         {"Upgrade addon for non-existent channel",
           ?_assertMatch({ok,{{_,404,_},_,"Not found"}},
               http:request(post, 
                   {"http://localhost:8002/channels/0/addon", headers(), content_type(), iolist_to_binary(mochijson2:encode({struct, [
                       {"addon", <<"advanced">>}
                   ]}))}, [], []))},
         {"Create session",
            fun() ->
         ...
         {"Create drain",
            ?_assertMatch({ok,{{_,201,_},_,"OK"}},
                http:request(post, 
                    {"http://localhost:8002/channels/" ++ get(channel_id) ++ "/drains", headers(), content_type(), iolist_to_binary(mochijson2:encode({struct, [
                        {"host", <<"10.0.0.1">>},
                        {"port", 514}
                    ]}))}, [], []))},
         ...
         {"Get info",
            fun() ->
                Ret = http:request(get, {"http://localhost:8002/channels/" ++ get(channel_id) ++ "/info", headers()}, [], []),
                InfoStr =
         ...
         {"Get drain list",
            ?_assertMatch({ok,{{_,200,_},_,"[{\"host\":\"10.0.0.1\",\"port\":514}]"}},
                http:request(get, {"http://localhost:8002/channels/" ++ get(channel_id) ++ "/drains", headers()}, [], []))},
         {"Input logs",
            fun() ->
                [begin
         ...
         {"Verify logs",
            fun() ->
                Ret = http:request(get, {"http://localhost:8002" ++ get(session), headers()}, [], []),
         ...
         {"Delete drain",
            ?_assertMatch({ok,{{_,200,_},_,_}},
                http:request(delete, {"http://localhost:8002/channels/" ++ get(channel_id) ++ "/drains?host=10.0.0.1&port=514", headers()}, [], []))},
         {"Delete non-existent drain",
            ?_assertMatch({ok,{{_,404,_},_,"Not found"}},
                http:request(delete, {"http://localhost:8002/channels/" ++ get(channel_id) ++ "/drains?host=10.0.0.1&port=514", headers()}, [], []))},
         {"Delete channel",
            ?_assertMatch({ok,{{_,200,_},_,_}},
                http:request(delete, {"http://localhost:8002/channels/" ++ get(channel_id), headers()}, [], []))},
         ...
         {"Verify healthcheck 'Zero child processes running'",
            fun() ->
                ...
                ?assertMatch({ok,{{_,500,_},_,"Zero logplex_read_queue_sup child processes running"}}, http:request(get, {"http://localhost:8002/healthcheck", headers()}, [], []))
            end}
        ]
    }.
