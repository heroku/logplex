...
main(_) ->
    ...
    {ok,{{_,201,_},_,Token}} = http:request(post, 
    ...
    {ok,{{_,201,_},_,HerokuToken}} = http:request(post, 
        {"http://localhost:8002/channels/" ++ ChannelId ++ "/token", headers(), content_type(), iolist_to_binary(mochijson2:encode({struct, [
            {"channel_id", list_to_binary(ChannelId)},
            {"name", <<"heroku">>}
        ]}))}, [], []),

    {ok,{{_,200,_},_,_}} = http:request(post, 
        {"http://localhost:8002/channels/" ++ ChannelId ++ "/addon", headers(), content_type(), iolist_to_binary(mochijson2:encode({struct, [
            {"addon", <<"advanced">>}
        ]}))}, [], []),

    {ok,{{_,201,_},_,Session}} = http:request(post, 
        {"http://localhost:8002/sessions", headers(), content_type(), iolist_to_binary(mochijson2:encode({struct, [
            {"channel_id", list_to_binary(ChannelId)},
            {"name", <<"app1@logplex.heroku.com">>}
        ]}))}, [], []),

    {ok,{{_,201,_},_,_}} = http:request(post, 
        {"http://localhost:8002/channels/" ++ ChannelId ++ "/drains", headers(), content_type(), iolist_to_binary(mochijson2:encode({struct, [
            {"host", <<"10.0.0.1">>},
            {"port", 514}
        ]}))}, [], []),
    
    {ok,{{_,200,_},_,_}} = http:request(get, {"http://localhost:8002/channels/" ++ ChannelId ++ "/info", headers()}, [], []),

    {ok,{{_,200,_},_,_}} = http:request(get, {"http://localhost:8002/channels/" ++ ChannelId ++ "/drains", headers()}, [], []),
    ...
    {ok,{{_,200,_},_,Logs}} = http:request(get, {"http://localhost:8002" ++ Session, headers()}, [], []),
    Logs1 = lists:flatten(["2010-11-10T17:16:33-08:00 app[web.1]: " ++ Msg ++ "\n" || Msg <- Msgs]),
    Logs1 = Logs,

    {ok,{{_,200,_},_,_}} = http:request(delete, {"http://localhost:8002/channels/" ++ ChannelId ++ "/drains?host=10.0.0.1&port=514", headers()}, [], []),

    {ok,{{_,200,_},_,_}} = http:request(delete, {"http://localhost:8002/channels/" ++ ChannelId, headers()}, [], []),
