...
UpgradeNode = fun () ->
  case logplex_app:config(git_branch) of
      "v58" ->
          io:format(whereis(user), "at=upgrade_start cur_vsn=58~n", []);
      "v59" ->
          io:format(whereis(user),
                    "at=upgrade type=retry cur_vsn=58 old_vsn=59~n", []);
      Else ->
          io:format(whereis(user),
  ...
  io:format(whereis(user), "at=upgrade_end cur_vsn=59~n", []),
  application:set_env(logplex, git_branch, "v59"),
  ok
end.
